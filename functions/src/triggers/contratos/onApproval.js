...
// ─────────────────────────────────────────────────────────────────────────
// DEPRECADO — envío del contrato a activaciones AL APROBAR.
//
// El envío del PDF/contrato a activaciones se movió a `onSerialesAsignadasSendPdf`,
// que se dispara DESPUÉS de que inventario asigna los seriales. Esta función se
// CONSERVA (no se borra todavía: borrado PENDIENTE) por si hay que revertir,
// pero está DESHABILITADA para no duplicar el correo ni enviarlo antes de tiempo.
// Para reactivar el comportamiento viejo: poner ENVIO_EN_APROBACION = true.
// ─────────────────────────────────────────────────────────────────────────
const ENVIO_EN_APROBACION = false;
const onContratoActivadoSendPdf = onDocumentUpdated(
  ...
  async (event) => {
    if (!ENVIO_EN_APROBACION) return null; // deshabilitado (ver nota arriba)
...
module.exports = {
  ...
  // Conservada pero deshabilitada (borrado pendiente) — ver nota arriba.
