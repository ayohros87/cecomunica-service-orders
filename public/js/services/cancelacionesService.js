...
const CancelacionesService = {
  ...
  // Cierre administrativo de la enmienda (solo sobre aprobadas). La
  // recuperación física va por la orden de DEVOLUCIÓN creada al aprobar:
  // check-in por serial → pool → ENTRADA de inspección.
  async cerrar(id, cerradoPorUid, { condicionNotas = '' } = {}) {
    const db = firebase.firestore();
    return db.collection('solicitudes_cancelacion').doc(id).update({
      estado: 'cerrada',
