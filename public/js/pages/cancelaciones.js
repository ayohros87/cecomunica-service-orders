...
window.Cancelaciones = {
  ...
  async cargarCola() {
    ...
    cont.innerHTML = rows.map(r => {
      ...
      if (r.estado === 'pendiente' && this.esAprobador()) {
        ...
        acciones = `<div style="display:flex; gap:8px; margin-top:8px;">
             <button class="btn sm btn-primary" onclick="Cancelaciones.cerrar('${r.id}')"><i data-lucide="package-check"></i> Cerrar enmienda</button>
           </div>`;
      }
  ...
  // Cierre administrativo de la enmienda. La recuperación física de los
  // equipos vive en la orden de DEVOLUCIÓN creada al aprobar (check-in por
  // serial en Órdenes → pool → ENTRADA de inspección); aquí ya no se
  // registran entradas ni se toca el pool.
  async cerrar(id) {
