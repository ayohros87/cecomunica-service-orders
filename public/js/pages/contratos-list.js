...
window.ContratosLista = {
  ...
  crearFila(id, data) {
    ...
    fila.innerHTML = `
      ...
      <td class="td-mono" style="text-align:right; color:var(--fg-1); font-weight:600;" title="${tot.tieneCargos ? 'Total mensual (incluye otros conceptos)' + (tot.tieneCargosUnicos ? ' · Primer pago ' + FMT.money(tot.primerPago) : '') : 'Total del contrato'}">${FMT.money(tot.totalConITBMS)}${tot.tieneCargos ? ' <span style="color:var(--accent); font-weight:700;" title="Incluye otros conceptos">＋</span>' : ''}</td>
