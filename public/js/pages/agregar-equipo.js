...
// SerialField deja un chip PERSISTENTE junto al campo (antes el aviso era un
// toast pasajero que se perdía escaneando con la cabeza agachada). El
// autocompletado del modelo se conserva vía onInfo.
container.addEventListener("focusout", (e) => {
  const input = e.target;
  if (!input.classList?.contains("serie")) return;
  if (typeof EquiposPoolService === "undefined" || typeof SerialField === "undefined") return;
  if (input._sfAdjuntado) return;

  const fieldset = input.closest("fieldset");
  const modeloSel = fieldset?.querySelector(".modelo");
  SerialField.adjuntar(input, {
    clienteId: () => ordenClienteId,
    modelo: () => {
      const m = modelos.find(x => x.id === modeloSel?.value);
      return m ? { modelo_id: m.id, modelo_label: m.nombre } : null;
    },
    onInfo: ({ docs, unidad }) => {
      if (!modeloSel || modeloSel.value) return;
      // Con colisión de serial entre modelos no se autocompleta (ambiguo).
      const u = unidad || (docs.length === 1 ? docs[0] : null);
      if (!u) return;
      const porId = modelos.find(m => m.id === u.modelo_id);
      const porNombre = porId ? null : modelos.find(m => normName2(m.nombre) === normName2(u.modelo_label || ""));
