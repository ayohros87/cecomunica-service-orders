...
**Inline `<style>` cleanup — `.btn.primary` strand shipped 2026-05-19:** the design call ("gradient or flat?") was resolved by consulting the design-system kit. `design-system/preview/components-buttons.html` and `ui_kits/app/app.css:197-208` both spec **flat** primary buttons (solid `var(--brand)`, one step darker on hover, translateY(1px) on press) and the README explicitly bans "playful gradients". Five inline `.btn.primary` overrides removed in `contratos/{nuevo-contrato,nuevo-cliente,index}.html`, `POC/{vendedores-batch,imprimir-equipos}.html`. Three of those were the gradient; two were already flat and just redundant. Pages now inherit the canonical from `ceco-ui.css:200`.
