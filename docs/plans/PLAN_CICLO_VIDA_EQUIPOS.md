...
6. **Orden de ENTRADA automática** (agregado 2026-07-16): toda entrada genera una orden de servicio tipo ENTRADA en la cola del taller (lib/ordenEntrada.js) — equipos precargados con la condición reportada, cliente y contrato vinculados, flag `entrada_inspeccion` para que onOrdenWritePool NO mueva el estado del pool (siguen en cuarentena; solo se enlaza `orden_actual_id`). Correo automático a recepción (empresa/config.email_recepcion o usuarios rol recepcion) + vendedor asignado del cliente. Disparadores: cierre de enmienda con entradas (idempotente vía `orden_entrada_id` en la solicitud) y anulación con seriales (ídem en el contrato). La disposición final sigue siendo por unidad en el pool (Inspección OK / baja) tras la intervención del técnico.
