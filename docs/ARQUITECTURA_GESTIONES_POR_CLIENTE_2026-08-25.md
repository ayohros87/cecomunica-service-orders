...
- **Un solo botón "Renovar cuenta"** (decisión 13 enmendada): el sistema decide
  el formato según el ancla — contrato completo en sistema → anexo de
  renovación; sin contrato o cascarón legacy sin líneas → **Contrato de
  Renovación** (documento fundacional, `origen_tipo:'regularizacion'`)
  precargado con la foto de la flota del pool (inventario POR SERIAL con
  columna de propiedad que el cliente firma reconociendo), tarifario completo,
  cláusula de no-novación y vigencia desde la firma; el legacy queda vinculado
  como origen y se cierra. Aprobación de gerencia en ambos formatos. Borrador
  visual en el artifact "Contrato de Regularización".
...
4. **Penalidad de baja (ENMENDADA 2026-08-27)** — se evalúa contra el tramo del ítem, con una regla unificada a dos niveles (aplica igual a bajas parciales por equipo y a la terminación total del contrato): tramo/contrato **no vencido** → penalidad de **3 meses de mensualidad**; **vencido** (en continuidad mes a mes) → **preaviso de 60 días** (facturando el servicio hasta el término del aviso) **más penalidad de 30 días** de mensualidad. La gestión de baja calcula y muestra la liquidación **por contrato afectado**.
...
13. **Regularización = renovación directa (2026-08-27, enmendada el mismo día)** — la regularización NO es un flujo aparte: se ejecuta **como renovación**. Un solo botón "Renovar cuenta" y el sistema decide el formato: si la cuenta tiene contrato completo en sistema → **anexo de renovación**; si no tiene contrato (o solo un cascarón legacy sin líneas) → **Contrato de Renovación** = el documento fundacional de la cuenta, precargado con la **foto real de la flota del pool** (inventario POR SERIAL que el cliente firma reconociendo), cláusula de no-novación, tarifario completo y vigencia desde la firma; el cascarón legacy queda vinculado como origen y se cierra. Racional: la mayoría de esos clientes ya tiene el contrato vencido — para ellos esto ES una renovación, no un concepto nuevo. **Aprobación de gerencia obligatoria** en ambos formatos. Borrador visual: artifact "Contrato de Regularización" (formato contrato fundacional).
...
15. **Seriales en el documento: NUNCA inferidos (2026-08-27)** — el Anexo A de inventario solo imprime seriales **colocados explícitamente por bodega** (contratos con asignación formal; la captura de seriales apenas empezó hace meses). Cuentas sin contrato, legacy o contratos de sistema sin seriales asignados: el Anexo A se genera con **filas en blanco + columna de verificación** y se levanta físicamente con el cliente al firmar; bodega registra después lo verificado por el flujo normal (y ahí queda amarrado). El pool migrado nunca es fuente del documento. **Manejo multipágina**: cuerpo del contrato fijo en 2 páginas; el inventario vive en el Anexo A con encabezado repetido por página (print `thead: table-header-group`), ~30 filas/página, folio "Anexo A · contrato · Pág. X/Y", **iniciales de ambas partes en cada página**, firma completa solo en la última, y resumen por modelo con **valor de reposición desde el catálogo** (reemplaza los precios hard-coded de la cláusula actual). Las 10 mejoras al contrato vigente (bug [[DURACION]], renovación automática vs anexos, garantía 6 vs 12 meses, precios congelados, referencias "al dorso", cláusulas sin numerar, contradicción de propiedad, etc.) están en el artifact "Contrato de Renovación" parte 3.
